...
--We should explain clearer what the different training modes are - unnormalised vs normalised, and normal vs adversarial. This could be explained with a table, or CORA normalised.
...
--Contributions! But SAIV readers will probably be okay with ML component.
...
--Consider putting training before verification. Say that it is known that normal training is not robust - don't need to define robustness before training.
...
--Make more use of equations table, mention that they are implemented in CORA. Mention in contributions - coding complex dynamics in CORA
--Dedicate section/subsection explaining difficulty in formalising in CORA (impossibility in vehicle/marabou). Include snippet of code?
...
--Could have section (3?) "formalisation of alsomitra in NNVs", discuss formalising either way is difficult, challenges, variables, nonlinearity, snippets. Vehicle is also difficult, no equations but infinite horizon, made certain choices, etc.
...
--Maybe clarify contributions. Argue that improvements will make cohesive result - where exactly should the problem be completed, or continued in future?
--Need to talk more about it, inc with Alessandro. Is problem complete or not? Can it even be solved? To what extent?
--Ideal property is infinite horizon, we think two tools are good approximators - cite other papers and how they use approximations
--Conceptually, is infinite horizon verification possible or reasonable?
...
--Same as review 1. Maybe there is a better way of conceptualising
...
--Same as review 1. We should strengthen use of equations + snippets as contribution instead of in appendix
--Table 1 is original contribution, cross-reference draft
...
--Reachable sets need to be explained in formalisation section. 
--Explain how normalisation is not available in CORA
...
--Explain adversarial training for regression better - how much needed modification
--Explain how LR is handled - measured etc.
...
--Explain discretization of reachability in formalisation section - time has to be discretised etc.
...
--See if we can emphasize difficulty in formalisation as contribution
...
--Change table to be more readable, fix formatting
...
--Verification definitions in marabou work for any other verifier
--Justify use of vehicle - makes complex property definition easier. Normalisation should be done in vehicle, and explained
--Vehicle allows definition of relational properties, talk about line and output. Ideal property would be exactly relational - NN will follow close to line, irrespective of coordinates.
...
--Read through, find links/similarities/differences and cite if useful. Statement of where we deviate
