...
def train(model, train_dataset, test_dataset, epochs, batch_size, pgd_steps, hyperrectangles, hyperrectangles_labels, alpha=1, beta=0, eps_multiplier=1, from_logits=False, optimizer=keras.optimizers.Adam()):    
    optimizer = optimizer
    ...
    for epoch in range(epochs):
        ...
        for x_batch_train, y_batch_train in train_dataset:
            # NEW
            y_batch_train = tf.reshape(y_batch_train, [y_batch_train.shape[0], 1])
        ...
        for x_batch_test, y_batch_test in test_dataset:
            ...
            # NEW
            test_outputs = tf.reshape(test_outputs, [1,test_outputs.shape[0]])
            y_batch_test = tf.reshape(y_batch_test, [y_batch_test.shape[0], 1])
            
            test_acc_metric.update_state(y_batch_test, test_outputs)
            
            # NEW
            test_outputs = tf.reshape(test_outputs, [test_outputs.shape[1],1])
        ...
        for x_batch_test, y_batch_test in pgd_dataset:
            ...
            pgd_outputs = tf.reshape(pgd_outputs, [1,pgd_outputs.shape[0]])
            pgd_acc_metric.update_state(y_batch_test, pgd_outputs)
            
            pgd_outputs = tf.reshape(pgd_outputs, [pgd_outputs.shape[1],1])
            y_batch_test = tf.reshape(y_batch_test, [y_batch_test.shape[0], 1])
