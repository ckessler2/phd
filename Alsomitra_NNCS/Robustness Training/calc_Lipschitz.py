...
counterexample = [ 0.177744, 0.0, 0.439282, 0.0, 0.0, 0.101906, 0.0, 0.177744, 0.0, 0.439282, 0.0, 0.0, 0.101906, 0.0 ]
...
L = abs(y2-y1)/(abs(x2[0]-x1[0]))
L = L[0]
print("L (base model) = " + str(L[0]))

y3 = k_model2.predict(np.expand_dims(x1, axis=0))
y4 = k_model2.predict(np.expand_dims(x2, axis=0))

L = abs(y3-y4)/(abs(x2[0]-x1[0]))
L = L[0]
print("L (adversarial model) = " + str(L[0]))


y5 = k_model3.predict(np.expand_dims(counterexample, axis=0))
L = abs(y5.flat[0]-y5.flat[1])/(abs(x2[0]-x1[0]))
print("L (base model merged) = " + str(L))