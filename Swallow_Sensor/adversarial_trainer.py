...
# def SCCE_LOSS(y, y_ad):
#     y = tf.cast(y,tf.float32)
#     y_ad = tf.cast(y_ad,tf.float32)
    
#     L = np.float32(keras.losses.SparseCategoricalCrossentropy(y, y_ad))
#     return L
...
class AdversarialTrainer:
    ...
    def train_with_adversarial_examples(self, train_dataset, epochs=10, callbacks=None, alpha=1,  class_weights = {0: 1.0, 1: 1.19, 2: 1.81, 3: 1.61}):
        """
        ...
        for epoch in range(epochs):
            ...
            for x_batch, y_batch in train_dataset:
                ...
                history = self.model.train_on_batch(combined_x,combined_y,class_weight=class_weights)
                ...
                with tf.GradientTape() as tape:
                    ...
                    ce_batch_loss = keras.losses.SparseCategoricalCrossentropy(from_logits=True)
                    
                    ce_loss_value = ce_batch_loss(y_batch, adversarial_y_batch) # Calculate loss
                    loss = ce_loss_value * alpha
