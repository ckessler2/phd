...
function [data,errors,ex_all,ds] = simulate(y0,parameters, ObjectiveFunction,nn, nnc)
    ...
    for i = 1:num_sims
        ...
        if nnc == true
            ...
            ex = 0.1870 + (error * 0.1) + (derivative * -0.9) + (integral * 0.00015);
            if ex > 0.19
                ex = 0.193;
            elseif ex < 0.181
                ex = 0.181;
        ...
        [v_xp, v_yp, omega, theta, x, y, error2,tSol] = ObjectiveFunction([parameters,ex],v_xp0, v_yp0, omega0, theta0, x0, y0,num_sims);
...
function [v_xp, v_yp, omega, theta, x_, y_, error,tSol] = Alsomitra_nondim(opt,v_xp0, v_yp0, omega0, theta0, x0, y0,num_sims)
    ...
    f = 999;
    % number of periods
    n = 1 / num_sims; % Cathal's % n = 1; % mine
    ...
    [tSol, ySol] = ode45(@(t, y) nondimfreelyfallingplate2_limited(p, y, opt(1:10)), t, Y0);

    % ysoldiff  = abs(ySol-ySol2)./ySol
